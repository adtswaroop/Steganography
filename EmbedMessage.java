...
public class EmbedMessage extends JFrame implements ActionListener {
    ...
    JLabel numText= new JLabel("Generated Key is :");
    JLabel numTextField = new JLabel("");
    ...
    private void embedMessage(BufferedImage img, String mess) {
        ...
        embedInteger(img, messageLength, 0, Integer.parseInt(strArr[indexToUse++]+""));

        byte b[] = mess.getBytes();
        for (int i = 0; i < b.length; i++) {
            embedByte(img, b[i], i * 8 + 32, Integer.parseInt(strArr[indexToUse++]+""));
            if(indexToUse==6)
                indexToUse=0;
        }
    }
    ...
    public String getNum(){
        ...
        for (int i = 0; ; i++) {
            if (num.length() == 6)
                break;
            random1 = random.nextInt(7);
            num += (random1 > 3 ? random1 : "");
        }
        return num;
    }
