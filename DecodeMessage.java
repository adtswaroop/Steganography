...
public class DecodeMessage extends JFrame implements ActionListener {
    ...
    JLabel numText= new JLabel("Key");
    JTextField numTextField = new JTextField("0",6);
    JTextArea message = new JTextArea(10, 3);
    BufferedImage image = null;
    JScrollPane imagePane = new JScrollPane();
    String num;

    public DecodeMessage(String num) {
    ...
    private void decodeMessage() {
        num = (numTextField.getText()==null || numTextField.getText().isEmpty())?"0":numTextField.getText() ;

        int indexToUse=0;
        char[] strArr= this.num.toCharArray();

        int len = extractInteger(image, 0, Integer.parseInt(strArr[indexToUse++]+""));
        byte b[] = new byte[len];
        for (int i = 0; i < len; i++) {
            b[i] = extractByte(image, i * 8 + 32, Integer.parseInt(strArr[indexToUse++] + ""));
            if(indexToUse==strArr.length)
                indexToUse=0;
        }
